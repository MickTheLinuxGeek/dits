...
function validateConfig() {
    ...
    if (exports.config.app.env === 'production') {
        requiredVars.push('JWT_SECRET', 'JWT_REFRESH_SECRET', 'SESSION_SECRET', 'ENCRYPTION_KEY', 'DATABASE_URL');
    }
    const missing = requiredVars.filter((varName) => !process.env[varName] || process.env[varName] === '');
    if (missing.length > 0) {
    ...
    if (exports.config.app.env === 'production') {
        ...
        const insecureSecrets = devSecrets.filter((secret) => exports.config.jwt.secret.includes(secret) ||
            exports.config.jwt.refreshSecret.includes(secret) ||
            exports.config.session.secret.includes(secret) ||
            exports.config.encryption.key.includes(secret));
        if (insecureSecrets.length > 0) {
            throw new Error('Production environment detected with insecure development secrets. Please set proper secret values.');
        }
    }
}
/**
...
function isDevelopment() {
    return exports.config.app.env === 'development';
}
/**
 * Returns true if running in production mode
 */
function isProduction() {
    return exports.config.app.env === 'production';
}
/**
 * Returns true if running in test mode
 */
function isTest() {
    return exports.config.app.env === 'test';
}
